...
.scroll-progress {
  ...
  left: 0;
  height: 3px;
  background: linear-gradient(90deg, var(--accent) 0%, #00f0ff 100%);
  width: 0%;
  z-index: 10001;
...
.blob-2 {
  ...
  animation: floatBlob2 30s infinite ease-in-out alternate 2s;
}
...
.blob-3 {
  ...
  animation: floatBlob3 20s infinite ease-in-out alternate 5s;
}
...
.project-card:hover::before {
  background: linear-gradient(135deg, rgba(255, 51, 102, 0.4) 0%, rgba(0, 240, 255, 0.4) 100%);
}

.project-card .card-bg {
  position: absolute;
  top: 0; left: 0; width: 100%; height: 100%;
  background: radial-gradient(circle 250px at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(255, 51, 102, 0.12) 0%, transparent 80%);
  opacity: 0;
  transition: opacity 0.5s ease;
  z-index: 1;
}
